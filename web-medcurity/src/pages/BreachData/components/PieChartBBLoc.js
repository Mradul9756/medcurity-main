...
const PieChartBBLoc = ({graphContainer, filterBreachData, startDate, endDate}) => {
    ...
    function getDataPoints() {
        ...
        dateFilteredData.forEach(breach => {
            ...
            const breach_location = breach['location_of_breached_information'];
            const indivuduals_affected = breach['individuals_affected'];
            // If it doesnt exist, then push and add indivduals affected
            if(!locationLabels.includes(breach_location)){
                locationLabels.push(breach_location);
                indvidualsAffected.push(indivuduals_affected)
            } 
            else {
                // Get the index of the location
                const index = locationLabels.indexOf(breach_location);
    ...
    const options = {
        ...
        plugins: {
            ...
            legend: {
                display: true,
                position: 'right', 
            },// displays what each line represents
        },
        layout: {
            padding: 40,
        }
